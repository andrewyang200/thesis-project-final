...
re-weighted composition-adjusted estimate is slightly amplified relative
to the raw association---though this comparison rests on point estimates without formal standard errors for the MSM specification---indicating that concurrent
compositional shifts worked against the dismissal pattern
rather than inflating it. For settlement, approximately half
the raw suppression is associated with observable case-mix
changes. These estimates are not causal---the 1992 placebo
test, the post-treatment bias concern, and the fundamental
overlap problem (ESS = 578 pre-PSLRA controls versus
$\sim$12,000 post-PSLRA cases) preclude that
interpretation---but they provide a principled sensitivity
check unavailable in prior work.

Third, circuit geography emerges as the dominant structural
predictor of case trajectories, with settlement hazard ratios
ranging from 4.1 to 17.6 times the Second Circuit baseline
across circuits (Table~\ref{tab:cox_ext_settle}). The shared frailty
models are suggestive of meaningful unobserved heterogeneity
in settlement outcomes even after controlling for
observable covariates---though the 11-cluster constraint limits precision---while dismissal heterogeneity is
...
circuit level ($\hat{\theta} = 0.449$; note this estimate carries substantial imprecision given only 11 clustering units), but cannot identify its source.
