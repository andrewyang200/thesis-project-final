...
(Section~\ref{sec:results_iptw}) separates the raw PSLRA
association into a component linked to observable case-mix
shifts and a residual component that remains after
adjustment. In point-estimate terms, the dismissal
association strengthens modestly after composition
adjustment (1.414 to 1.519), suggesting that observable
compositional shifts partially offset the raw dismissal
association. The settlement estimate attenuates from 0.557
to 0.741, implying that observed case-mix differences
account for about 41\% of the raw settlement gap, but the
residual settlement association does not survive the
cluster-robust specification ($p = 0.158$) or the two
largest circuit-specific sub-samples. These estimates are
composition-adjusted rather than causally identified, but
they provide a principled sensitivity check unavailable in
prior work.
...
settlement association's fragility under circuit-specific
decomposition as binding interpretive limits rather than as
