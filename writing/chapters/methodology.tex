...
requiring a competing-risks framework throughout.
...
The CIF for event type $k \in \{1,2\}$ is:
\begin{equation}
...
\end{equation}
where $I_1 = [0,1)$, $I_2 = [1,2)$, and $I_3 = [2,\infty)$. The cutpoints at $t = 1$ and $t = 2$ years were chosen heuristically to align with the typical procedural timeline of a motion to dismiss in securities class actions: the first year captures the pleading-stage gauntlet; the second year spans the interlocutory period before full discovery. These cutpoints are not data-derived and do not represent formal structural break estimates. The piecewise specification is applied symmetrically to both the settlement and dismissal models to allow direct comparison of temporal patterns across outcomes. Note that the Grambsch-Therneau test does not reject proportional hazards for the PSLRA indicator in the dismissal model ($p = 0.120$; see Section~\ref{sec:results_diagnostics}); the piecewise decomposition is motivated primarily by the settlement model's significant violation ($p < 0.001$, denoting a statistical failure of the proportional hazards assumption) and by the substantive interest in comparing temporal dynamics across both outcomes. Proportional hazards assumption violations (i.e., statistical failures of the constant-HR assumption, not legal infractions) in circuit, origin, MDL, and statutory basis in the extended models are documented in the diagnostics section of Chapter~\ref{ch:results} and addressed via stratification where feasible.
